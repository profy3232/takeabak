...
func UpgradeGoPix(isUpgrade bool) {
	fmt.Println("\033[0;34m🔄 Starting GoPix upgrade...\033[0m")

	homeDir, err := os.UserHomeDir()
	if err != nil {
		fmt.Println("\033[0;31m❌ Failed to detect home directory:\033[0m", err)
		return
	}
	...
	if _, err := os.Stat(tmpDir); os.IsNotExist(err) {
		fmt.Println("\033[0;33m📥 Cloning GoPix repository...\033[0m")
		cmd := exec.Command("git", "clone", "--depth=1", repoURL, tmpDir)
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
			fmt.Println("\033[0;31m❌ Failed to clone repository:\033[0m", err)
			return
		}
	} else {
		fmt.Println("\033[0;34m🔁 Repository exists. Checking for updates...\033[0m")
		cmd := exec.Command("git", "-C", tmpDir, "pull")
		var output bytes.Buffer
		cmd.Stdout = &output
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
			fmt.Println("\033[0;31m❌ Failed to pull latest changes:\033[0m", err)
			return
		}
	...
	fmt.Println("\033[0;34m🚀 Running install.sh ...\033[0m")
	...
	cmd.Stdin = os.Stdin
	if err := cmd.Run(); err != nil {
		fmt.Println("\033[0;31m❌ Installation failed:\033[0m", err)
		return
	}
}
